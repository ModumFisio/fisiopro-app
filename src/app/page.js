...
import { 
  Activity, Users, FileText, Settings, LogOut, Plus, Search, 
  ChevronRight, ChevronDown, Save, Play, CheckCircle, BarChart2, 
  ArrowLeft, Download, Trash2, Edit, Printer, FileOutput, Link as LinkIcon, 
  Copy, Mail, Send, Menu, X, AlertCircle 
} from 'lucide-react';
...
const SEED_TESTS = [
  {
    ...
    category: "Cap i Coll", 
    description: "Avalua l'impacte del mal de cap en la vida diària. Cada resposta té un valor determinat.",
    questions: [
      { 
        id: "h1", text: "1. Quan té mal de cap, ¿amb quina freqüència el dolor és fort?", 
        type: "choice", required: true, 
        options: [ { text: "Mai", points: 6 }, { text: "Gairebé mai", points: 8 }, { text: "A vegades", points: 10 }, { text: "Molt sovint", points: 11 }, { text: "Sempre", points: 13 } ] 
      },
      { 
        id: "h2", text: "2. ¿Amb quina freqüència el mal de cap li limita la capacitat per fer les activitats diàries habituals?", 
        type: "choice", required: true, 
        options: [ { text: "Mai", points: 6 }, { text: "Gairebé mai", points: 8 }, { text: "A vegades", points: 10 }, { text: "Molt sovint", points: 11 }, { text: "Sempre", points: 13 } ] 
      },
      { 
        id: "h3", text: "3. Quan té mal de cap, ¿amb quina freqüència desitja estirar-se?", 
        type: "choice", required: true, 
        options: [ { text: "Mai", points: 6 }, { text: "Gairebé mai", points: 8 }, { text: "A vegades", points: 10 }, { text: "Molt sovint", points: 11 }, { text: "Sempre", points: 13 } ] 
      }
    ],
...
const ModumLogo = ({ className, isPrint = false }) => {
  ...
  const sources = [ "/logo.jpg", "/logo.png", "/logo.jpg.jpg", "/logo.jpeg", "/MODUM-Logo-04 (16).jpg" ];
...
const SimpleLineChart = ({ data, xKey, yKey, title }) => {
  ...
  const points = data.map((d, i) => { 
    ...
    const y = height - padding - (((d[yKey] || 0) / maxScore) * (height - padding * 2)); 
    return `${x},${y}`; 
  }).join(' ');
        ...
        {data.map((d, i) => {
          ...
          return (
            <g key={i}>
              ...
              <text x={x} y={y - 12} fontSize="12" fontWeight="bold" fill="#2563eb" textAnchor="middle">{d[yKey]}</text>
              <text x={x} y={height - 5} fontSize="10" fill="#6b7280" textAnchor="middle">{d[xKey]}</text>
            </g>
          );
        })}
      </svg>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubResponses = onSnapshot(responsesRef, (snapshot) => {
      setResponses(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })).sort((a, b) => b.date - a.date));
    });
        ...
        <Card className="max-w-md w-full p-8 shadow-2xl border-t-8 border-[#5E3236]">
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            <div>
              ...
              <input type="email" required className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#5E3236] outline-none" value={loginEmail} onChange={e => setLoginEmail(e.target.value)} placeholder="hola@modum.cat" />
            </div>
            <div>
              <label className="block text-sm font-bold text-gray-700 mb-1">Contrasenya</label>
              <input type="password" required className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#5E3236] outline-none" value={loginPassword} onChange={e => setLoginPassword(e.target.value)} placeholder="••••••••" />
            </div>
            {authError && <p className="text-red-500 text-sm font-medium bg-red-50 p-2 rounded">{authError}</p>}
            <button type="submit" className="w-full py-3 text-lg mt-4 bg-[#5E3236] hover:bg-[#4a272a] text-white rounded-lg font-bold shadow-lg">Iniciar Sessió Segura</button>
          </form>
        </Card>
      </div>
    );
      ...
      <main className="flex-1 flex flex-col h-full md:h-screen overflow-hidden print-container print:h-auto print:overflow-visible print:block">
        <header className="bg-white border-b border-gray-200 px-8 py-4 justify-between items-center hidden md:flex no-print">
          ...
          <div className="flex items-center gap-4">
             <span className="text-sm text-gray-500">{new Date().toLocaleDateString('ca-ES')}</span>
          </div>
        </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="p-6">
          <div className="flex justify-between items-start">
            <div><p className="text-sm text-gray-500 font-medium">Total Pacients</p><h3 className="text-3xl font-bold mt-2">{patients.length}</h3></div>
            <div className="bg-blue-50 p-3 rounded-lg text-blue-600"><Users /></div>
          </div>
        </Card>
        <Card className="p-6">
          <div className="flex justify-between items-start">
            <div><p className="text-sm text-gray-500 font-medium">Tests Realitzats</p><h3 className="text-3xl font-bold mt-2">{responses.length}</h3></div>
            <div className="bg-green-50 p-3 rounded-lg text-green-600"><CheckCircle /></div>
          </div>
        </Card>
        <Card className="p-6">
          <div className="flex justify-between items-start">
            <div><p className="text-sm text-gray-500 font-medium">Tests a la Biblioteca</p><h3 className="text-3xl font-bold mt-2">{tests.length}</h3></div>
            <div className="bg-purple-50 p-3 rounded-lg text-purple-600"><FileText /></div>
          </div>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <div className="p-4 border-b border-gray-100 flex justify-between items-center">
            <h3 className="font-semibold text-lg">Últims Resultats</h3>
            <Button variant="outline" onClick={() => onNavigate('patients')}>Veure tots</Button>
          </div>
            ...
            {recentResponses.length === 0 ? <p className="text-gray-400 text-center py-6">Cap resultat registrat encara.</p> : recentResponses.map(res => {
              ...
              return (
                <div key={res.id} className="p-4 flex flex-col sm:flex-row sm:items-center justify-between hover:bg-gray-50 cursor-pointer gap-2" onClick={() => onNavigate('patient-detail', { id: res.patientId })}>
                  <div>
                    <p className="font-medium">{pat ? `${pat.firstName} ${pat.lastName}` : 'Desconegut'}</p>
                    <p className="text-sm text-gray-500">{test?.title || 'Test Eliminat'} ({res.episodeName || 'General'})</p>
                  </div>
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between gap-4">
        <div className="relative flex-1 max-w-md">
          ...
          <input type="text" placeholder="Cercar pacients..." className="w-full pl-10 pr-4 py-2 rounded-lg border focus:ring-2 focus:ring-blue-500 outline-none" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        </div>
      ...
      <Card className="overflow-x-auto w-full">
        <table className="w-full text-left border-collapse min-w-[500px]">
          <thead className="bg-gray-50 border-b">
            <tr><th className="p-4">Nom</th><th className="p-4">Telèfon</th><th className="p-4 text-right">Accions</th></tr>
          </thead>
          <tbody className="divide-y divide-gray-100">
            {filtered.map(p => (
              <tr key={p.id} className="hover:bg-blue-50 cursor-pointer" onClick={() => onNavigate('patient-detail', { id: p.id })}>
                <td className="p-4 font-medium">{p.firstName} {p.lastName}</td>
                <td className="p-4 text-gray-500">{p.phone || '-'}</td>
                <td className="p-4 text-right">
                  <Button variant="outline" className="ml-auto p-2"><ChevronRight className="w-4 h-4" /></Button>
                </td>
              </tr>
            ))}
          ...
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-md">
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div className="flex justify-end gap-3 mt-6">
                <Button variant="outline" onClick={() => setShowModal(false)}>Cancel·lar</Button>
                <Button type="submit"><Save className="w-4 h-4"/> Guardar</Button>
              </div>
            </form>
          </div>
        </div>
...
const PatientDetail = ({ patientId, patients, responses, tests, onNavigate, user }) => {
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto printable-area">
      {/* CAPÇALERA IMPRESSIÓ GLOBAL / EPISODI */}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-6 no-print">
        <div className="flex items-center gap-4">
          <button onClick={() => onNavigate('patients')} className="p-2 bg-gray-100 rounded-full hover:bg-gray-200">
        ...
        <div className="sm:ml-auto flex flex-wrap gap-2">
          ...
          <Button variant="danger" onClick={() => setDeleteModal({ type: 'patient', id: patient.id, name: `el pacient ${patient.firstName} ${patient.lastName}` })}><Trash2 className="w-4 h-4" /> Esborrar</Button>
        </div>
      </div>

      <Card className="p-6 border-t-4 border-blue-600 print-break-inside-avoid no-print">
        <div className="flex flex-col sm:flex-row sm:items-center gap-6">
          <div className="w-16 h-16 bg-blue-100 text-blue-700 rounded-full flex items-center justify-center text-2xl font-bold shrink-0 mx-auto sm:mx-0">
      ...
      {allEpisodes.length === 0 ? (
         <div className="p-8 md:p-12 text-center text-gray-500 bg-white rounded-xl border border-dashed flex flex-col items-center mx-2 no-print">
            ...
            <div key={episode} className={`bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden transition-all print:border-none print:shadow-none print:mb-8 ${index > 0 && printFilter === 'all' ? 'print-page-break' : ''}`}>
              ...
              >
                ...
                <div className="actions-group flex flex-wrap items-center gap-2 no-print w-full xl:w-auto ml-8 xl:ml-0">
                  ...
                  <Button className="flex-1 xl:flex-none text-sm" onClick={() => { setTargetEpisode(episode); setTestSelectorMode('run'); setShowTestSelector(true); }}><Play className="w-4 h-4" /> Fer Test</Button>
      ...
      <div className="space-y-0.5">
        ...
        <div className="grid grid-cols-1 gap-x-6 gap-y-0">
          {test?.questions.map((q, idx) => {
            ...
            if (q.type === 'choice' && ansVal !== undefined) {
               const opt = q.options && q.options[ansVal];
               ansText = opt ? `${opt.text} (${opt.points || 0} pts)` : 'Opció no trobada';
            }
              ...
              const selected = Object.keys(ansVal)
                 .filter(k => ansVal[k])
                 .map(k => q.options && q.options[k] ? q.options[k].text : 'Opció esborrada');
        ...
        {filteredTests.map(test => (
          <Card key={test.id} className="flex flex-col">
            <div className="p-5 flex-1">
              <span className="text-xs font-semibold uppercase tracking-wider text-blue-600 bg-blue-50 px-2 py-1 rounded mb-3 inline-block">{test.category || 'General'}</span>
              <h3 className="font-bold text-lg mb-2">{test.title}</h3>
              <p className="text-sm text-gray-600 line-clamp-3 mb-4">{test.description}</p>
            </div>
            <div className="p-4 bg-gray-50 border-t flex gap-2">
              <Button variant="outline" className="flex-1 text-sm py-1.5" onClick={() => onNavigate('test-builder', { id: test.id })}><Edit className="w-4 h-4" /> Editar</Button>
            </div>
          </Card>
        ))}
      </div>
...
const TestBuilder = ({ testId, tests, onNavigate, user }) => {
  ...
  const handleSave = async () => {
    ...
    try { 
      const testsRef = collection(db, 'biblioteca_tests_clinics_v3'); 
      if (testId === 'new') await addDoc(testsRef, test); 
      else await updateDoc(doc(testsRef, testId), test); 
      onNavigate('tests'); 
    } catch (error) { 
      console.error(error); 
    }
...
const TestRunner = ({ testId, patientId, episodeName, tests, user, onNavigate, isPatientMode = false }) => {
  ...
  const calculateScore = () => {
    ...
    if (!test.questions) return { total: 0, maxTotal: 0, interpretation: null };

    test.questions.forEach(q => {
      const answer = answers[q.id];
      if (q.type === 'choice') {
        const maxPts = q.options && q.options.length > 0 ? Math.max(...q.options.map(o => o.points || 0), 0) : 0;
        maxTotal += maxPts;
        if (answer !== undefined && q.options && q.options[answer]) total += q.options[answer].points || 0;
      } else if (q.type === 'checkbox') {
        const maxPts = q.options ? q.options.reduce((sum, o) => sum + (o.points > 0 ? o.points : 0), 0) : 0;
        maxTotal += maxPts;
        if (answer && q.options) {
          Object.keys(answer).forEach(k => { if (answer[k] && q.options[k]) total += q.options[k].points || 0; });
        }
  ...
  const handleSubmit = async () => {
    ...
    const finalResult = { 
      patientId, testId, episodeName, date: Date.now(), 
      answers, score: total, maxScore: maxTotal, 
      interpretation, submittedBy: isPatientMode ? 'patient' : 'physio' 
    };
    try {
